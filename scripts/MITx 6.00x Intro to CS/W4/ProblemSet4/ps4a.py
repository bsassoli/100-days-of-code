...
def getWordScore(word, n):
    ...
    for index in range(len(word)):
        score += SCRABBLE_LETTER_VALUES[word[index]]
...
def isValidWord(word, hand, wordList):
    ...
    for letter in word:
        if letter in hand:
            ...
            return False
    return (word in wordList) and (sum([counts[letter] <= hand[letter] for letter in word]) == len(word))
        ...
        while True:
            ...
            # Otherwise (the input is not a single period):
                ...
                if not isValidWord(user_inp, hand, wordList):
                    ...
                    print('Sorry, but {} is invalid!'.format(user_inp))
                    print()
                    break
                # Otherwise (the word is valid):
                else:
                    # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                    # updated = updateHand(hand, user_inp)
                    word_score = getWordScore(user_inp, n)
                    score += word_score
                    print("{}'s score is {} and player's score is {}".format(user_inp, getWordScore(user_inp, n), score))
                    print()
                # Update the hand
